...
export const CitationModal: React.FC<CitationModalProps> = ({ field, onClose, onConfirmField }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/60 backdrop-blur-xs">
      <div className="w-full max-w-xl rounded border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-xl p-4 sm:p-5 space-y-4 text-xs max-h-[92vh] sm:max-h-[90vh] overflow-y-auto">
        {/* Header */}
        ...
        <div className="flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
          ...
          {field.isConfirmed ? (
            <span className="justify-center sm:justify-start px-3 py-1.5 rounded bg-emerald-100 dark:bg-emerald-950/60 text-emerald-800 dark:text-emerald-300 font-medium font-mono text-[11px] flex items-center gap-1.5 border border-emerald-300 dark:border-emerald-800">
            ...
            onConfirmField && (
              <button
                ...
                className="w-full sm:w-auto justify-center px-4 py-1.5 rounded bg-emerald-600 hover:bg-emerald-700 text-white font-medium flex items-center gap-1.5 cursor-pointer shadow-xs"
              >
