...
### Fixed
- **15 RPM Embedding Bottleneck (ADR-017)**: `manager.py:224` now bypasses retriever for `≤15 chunks` (small-medium docs): `batch_chunks = chunks` with 0 embedding calls; only `>15` does sparse `first 8 fields × top_k=3 → ≤12 deduped` (8 calls vs. previous `51`). This eliminates the burst that showed `15/15 success` yet `0 output tokens` in your AI Studio screenshot. Validated on `Test source/51-field contract`: `51/51 gemini, confidence 1.0`.
- **Generator Brace Corruption (ADR-017)**: `generator.py:34` `_build_replacement_map` now separates `direct` (raw `{{x}}→value`) + `normalized` (`x→value`) via `_find_placeholders`; `generator.py:61` `_replace_in_text` replaces whole enclosed `raw` sorted by length. No more `{{SPK/0847}}` residue; `test_mapping_generator.py:130` updated.
- **Render Free-Tier Resilience**: Frontend `uploadFiles()` now waits 6×5s when `502/503/timeout` before throwing `Backend is waking up (Render Hobby cold start ~60s)`; `page.tsx:99` wake polling `12×5s` with `Retry now`; `manager.py:133` catches `PdfExtractionError`; `DownloadView` direct asset branch for `demo-` sessions.
- **Extraction Reliability (ADR-017 hardening)**: `_call_gemini_with_fallback()` now blacklists on `503` immediately (`extractor.py:335`); `_call_gemini_rest` REST fallback (`httpx`+`urllib`, `extractor.py:517`) + 35s `asyncio.wait_for` on both SDK/REST (`extractor.py:565`); `use_fake` no longer requires `_HAS_GENAI` (`extractor.py:204`); `FakeExtractor` distinctive guard prevents hallucination drift (`0.79→1.00`).
